...
class Order
{
    ...
    {
        $sql = "SELECT DATE(delivered_at) as date, SUM(total) as revenue 
                FROM {$this->table} 
    ...
    /**
     * Get revenue by date range (based on delivered_at - sale date)
     */
    public function getRevenueByDateRange($startDate, $endDate)
    {
        $sql = "SELECT DATE(delivered_at) as date, SUM(total) as revenue 
                FROM {$this->table} 
                WHERE status = 'delivered' 
                AND DATE(delivered_at) BETWEEN :start_date AND :end_date
                GROUP BY DATE(delivered_at) 
                ORDER BY date ASC";
        $stmt = $this->db->prepare($sql);
    ...
    {
        $sql = "SELECT DATE_FORMAT(delivered_at, '%Y-%m') as month, SUM(total) as revenue 
                FROM {$this->table} 
                WHERE status = 'delivered' 
                AND delivered_at >= DATE_SUB(NOW(), INTERVAL :months MONTH)
